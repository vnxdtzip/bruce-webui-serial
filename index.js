...
async function taskReloader() {
  const timer = parseInt(eConfigAutoReload.value);
  if (timer <= 0 || !$(".dialog.navigator:not(.hidden)")) {
    AUTO_RELOAD_SCREEN = null;
    return;
  }
  await reloadScreen();
  AUTO_RELOAD_SCREEN = setTimeout(taskReloader, timer);
}
...
function autoReloadScreen() {
  ...
  const timer = parseInt(eConfigAutoReload.value);
  if (timer > 0) AUTO_RELOAD_SCREEN = setTimeout(taskReloader, timer);
}

eConfigAutoReload.addEventListener("change", () => {
  sessionStorage.setItem("navAutoReload", eConfigAutoReload.value);
  autoReloadScreen();
});

// Restore saved preference
(function () {
  const saved = sessionStorage.getItem("navAutoReload");
  if (saved !== null) {
    const opt = eConfigAutoReload.querySelector(`option[value="${saved}"]`);
...
async function renderTFT(data) {
  ...
  void isWiFiMenu; // (wifi-warning removed)
...
const _navCanvas = $(".navigator-canvas");

_navCanvas.addEventListener("click", async (e) => {
...
(function () {
  ...
  const clearBtn = document.querySelector(".term-clear-btn");

  let history = [], histIdx = -1, pendingInput = "";
  let busy = false;
  ...
  function appendResp(text, isErr) {
    if (!text.trim()) return;
    const el = document.createElement("div");
    el.className = isErr ? "term-err" : "term-resp";
    el.textContent = text;
    output.appendChild(el);
    scrollBottom();
  }
  ...
  async function send() {
    ...
    try {
      const res = await fetch("/cm", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "cmnd=" + encodeURIComponent(cmd),
      });
      const text = await res.text();
      appendResp(text, !res.ok);
    } catch (e) {
      appendResp("Error: " + e.message, true);
    }
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      e.preventDefault();
      if (histIdx === -1) pendingInput = input.value;
      if (histIdx < history.length - 1) histIdx++;
      input.value = history[histIdx] || "";
    } else if (e.key === "ArrowDown") {
      e.preventDefault();
      if (histIdx > 0) { histIdx--; input.value = history[histIdx]; }
      else if (histIdx === 0) { histIdx = -1; input.value = pendingInput; }
